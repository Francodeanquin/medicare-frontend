...
const Profile = ({ user }) => {
  ...
  useEffect(() => {
    ...
    setSelectedFile(null); // Reset preview when user data changes
  }, [user]);
  ...
  const handleFileInputChange = async (e) => {
    ...
    if (file) {
      ...
      setSelectedFile(data.url); // Set preview URL
  ...
  const submitHandler = async (event) => {
    ...
    try {
      ...
      navigate(`${BASE_URL}/users/profile/me`);
    } catch (error) {
      ...
      <form onSubmit={submitHandler}>
        <div className="mb-5">
          <input
            ...
            className="w-full pr-4 py-3 border-b border-solid border-[#0066ff61]
              focus:outline-none focus:border-b-primaryColor text-[16px]
        ...
        <div className="mb-5 flex items-center gap-3">
          <figure className="w-[60px] h-[60px] rounded-full border-2 border-solid border-primaryColor flex items-center justify-between">
            <img
              src={selectedFile || formData.photo || user.photo || imgUrl}
              alt="Profile"
